...
@section('content')
            ...
            <form class="row g-2 align-items-center" method="get" action="{{ route('staff.products.index') }}">
                <div class="col-lg-6">
                    <input type="text" name="q" value="{{ $q ?? request('q') }}" class="form-control"
                            placeholder="Tìm theo tên, loại sản phẩm, nhà cung cấp">
                </div>
            ...
            <table class="table align-middle mb-0 table-hover products-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                        @if($img)
                            <img class="thumb" src="{{ asset('assets/images/'.$img) }}" alt="{{ $name }}">
                        @else
                        ...
                        <td>
                        @php $badge = $stock <= 0 ? 'stock-bad' : ($stock < 5 ? 'stock-warn' : 'stock-ok'); @endphp
                        <span class="badge {{ $badge }}">{{ $stock }}</span>
                        </td>
                        <td class="text-truncate" title="{{ $ncc }}">{{ $ncc }}</td>
                        <td class="text-end">
                            <button
                            ...
                            <form action="{{ route('staff.products.destroy', $id) }}" method="post" class="d-inline form-delete">
                                @csrf @method('delete')
    ...
    <div class="modal fade" id="modalEdit" tabindex="-1" aria-hidden="true">
        ...
        <form id="formEdit" class="modal-content" method="post" enctype="multipart/form-data"
            ...
            <div class="col-md-6">
                <label class="form-label">Nhà cung cấp</label>
                <select id="e_supplier" name="MANHACUNGCAP" class="form-select">
                <option value="">-- Chọn nhà cung cấp --</option>
                @isset($suppliers)
                    @foreach($suppliers as $sup)
                    <option value="{{ $sup->MANHACUNGCAP }}">{{ $sup->TENNHACUNGCAP }}</option>
...
@push('scripts')
    <script>window.APP_IMAGE_BASE = "/assets/images/";</script>
