                ...
                <tbody>
                    @forelse($promotions as $p)
                        ...
                        <tr>
                            ...
                            <td>
                                <div>
                                {{ optional($p->NGAYBATDAU)->format('d/m/Y') }} - {{ optional($p->NGAYKETTHUC)->format('d/m/Y') }}
                                </div>
                            ...
                            <td class="text-end">
                                <button
                                    ...
                                    data-json='@json($p->DIEUKIEN_JSON)'
                                    data-products="{{ $p->sanphams->pluck('MASANPHAM')->implode(',') }}"
                                    title="Sửa"><i class="bi bi-pencil"></i>
                                </button>
            ...
            <form class="modal-content" method="post" action="{{ route('staff.promotions.store') }}">
                ...
                <div class="modal-body row g-3">
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="small-note">Flash Sale gợi ý 100</div>
                    </div>
                    <div class="col-md-3">
                        <label class="form-label">Mã khuyến mãi (Voucher)</label>
                        <input type="text" name="MAKHUYENMAI" id="c_code" class="form-control" placeholder="XUAN2025" required>
                        <div class="small-note">Chỉ bắt buộc khi Phạm vi = Voucher</div>
                    </div>
                    <div class="col-md-3">
                    ...
                    {{-- Điều kiện cho Voucher (ORDER) --}}
                    <div class="col-12 c-order-only">
                        <div class="row g-3">
                            <div class="col-md-4">
                    ...
                    {{-- Mục tiêu cho Product (PRODUCT) --}}
                    <div class="col-12 c-product-only">
                        <label class="form-label">Gán trực tiếp sản phẩm</label>
                        <select name="sanphams[]" id="c_products" class="form-select" multiple>
                    ...
                    <div class="col-12 c-product-only">
                        <div class="row g-3">
                            <div class="col-md-4">
                                <label class="form-label">Theo Danh mục</label>
                                <select name="madanhmucs[]" id="c_dms" class="form-select" multiple>
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="maloais[]" id="c_loais" class="form-select" multiple>
                                    @foreach($loais as $l)
                            ...
                            <div class="col-md-4">
                                ...
                                <select name="manccs[]" id="c_nccs" class="form-select" multiple>
                                    @foreach($nccs as $n)
                        ...
                        <div class="small-note mt-1">Bạn có thể chọn đồng thời nhiều tiêu chí, hệ thống sẽ hợp nhất danh sách SP.</div>
                    </div>
                </div>
        ...
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <form id="formEdit" class="modal-content" method="post" data-action-template="{{ route('staff.promotions.update', ':id') }}">
                ...
                <div class="modal-body row g-3">
                    ...
                    {{-- Product only --}}
                    <div class="col-12 e-product-only">
                        <label class="form-label">Gán trực tiếp sản phẩm</label>
                    ...
                    <div class="col-12 e-product-only">
                        <div class="row g-3">
                            <div class="col-md-4">
                                <label class="form-label">Theo Danh mục</label>
                                <select id="e_dms" name="madanhmucs[]" class="form-select" multiple>
                                    @foreach($danhmucs as $dm)
                                        <option value="{{ $dm->MADANHMUC }}">{{ $dm->TENDANHMUC }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                <label class="form-label">Theo Loại</label>
                                <select id="e_loais" name="maloais[]" class="form-select" multiple>
                                    @foreach($loais as $l)
                                        <option value="{{ $l->MALOAI }}">{{ $l->TENLOAI }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-4">
                                <label class="form-label">Theo Nhà cung cấp</label>
                                <select id="e_nccs" name="manccs[]" class="form-select" multiple>
                                    @foreach($nccs as $n)
                                        <option value="{{ $n->MANHACUNGCAP }}">{{ $n->TENNHACUNGCAP }}</option>
                                    @endforeach
                                </select>
